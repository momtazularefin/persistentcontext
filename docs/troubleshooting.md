...
| `PCP_DOCUMENTATION_ROOT_*`                                                                      | The selected documentation root contradicts inspection, is unsafe, or lacks an initial outcome document. | Reuse the inspected established root; otherwise use `docs/` and include a cataloged initial outcome document.            |
| `PCP_DOCUMENTATION_REGISTRY_*` or `PCP_OUTCOME_DOCUMENT_PATH_INVALID`                           | The document catalog is incomplete, duplicated, stale, or places an outcome outside its project root.    | Reconcile every surviving document path and keep outcome entries beneath their owning project's configured root.         |
...
| `PCP_STATE_C_ROOT_REVIEW_REQUIRED` or `PCP_STATE_C_ROOT_REVIEW_INVALID`                         | Detected foreign roots lack a complete evidence-backed scope review.                                     | Review every emitted root as `translate` or `project-owned`; do not omit, invent, or leave a root unresolved.            |
...
| `PCP_STATE_C_COVERAGE_INVALID` with `coverage-relocation-*`                                     | A reviewed relocation is missing its single safe project-owned destination or collides with scope.       | Choose one absent path outside `.pcp/`, translated roots, ignored/excluded trees, symlinks, and nested repositories.     |
...
Encrypted, binary, invalid UTF-8, unreadable, malformed structured, unrecognized, oversized, excluded, and symlinked sources inside a root selected for translation block State C removal. If the entire detected root is actually an embedded example, archive, or ordinary project tree, preserve it through an evidence-backed `project-owned` root review. Never use root scoping to hide live foreign context or mark unread material as represented.

Use `relocated` only when the exact bytes of a reviewed regular foreign file must remain as project-owned reference material after its useful current state has been absorbed. Give it one new destination and review the emitted `move` plus derived `rmdir` operations. Do not pre-create the target or manually delete the emptied foreign tree; either action invalidates the approved inventory and plan digest.
...
Full validation reads archive contents. `validate --archive-index-only` checks operational archive identities without treating archived prose as routine startup context. Archived events are checked for schema, integrity, and secrets but not for portability, because they are immutable and were already checked in full while active. A portability error on an archived event therefore means an engine older than `0.3.1` is doing the checking. Never edit an archived event to clear a diagnostic.
...
- `PCP_UPGRADE_ASSETS_MISSING`: the engine that ran has no release assets beside it, which almost always means it is the installed `.pcp/tools/pcp.mjs`. An installed engine only runs the version it belongs to. Run `upgrade` with the incoming release engine from the `build-pcp` skill.
