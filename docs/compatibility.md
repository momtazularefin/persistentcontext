...
| `antigravity`           | `antigravity`                   | Antigravity workspace rule         | `.agents/rules/pcp.md`            | Generated frontmatter sets `trigger: always_on`, the only activation Antigravity loads unconditionally. Antigravity also loads the shared `AGENTS.md`. |
...
Before `0.3.0`, `AGENTS.md` carried `--client codex`. Every product that loaded it received two PCP instructions disagreeing about its identity, and Antigravity followed the shared file and registered as Codex. Antigravity later confirmed that the shared file was the only PCP instruction it received: its `.agents/rules/pcp.md` rule had no activation frontmatter, and Antigravity loads a workspace rule unconditionally only when it sets `trigger: always_on`. This earlier defect attributed Antigravity's work to a Codex actor without a validation failure, because a declared label is the only identity signal PCP has. The `0.3.2` source adds the trigger, so Antigravity loads both files and they agree when that version is installed. The reconstruction test now loads every file each product loads and fails if any of them names a different product's label. Actor IDs registered under the wrong label before `0.3.0` stay as they are; history is immutable, so the fix applies to identities registered from here on.

The Antigravity activation key comes from Antigravity's built-in customization documentation, which ships with the product and states that only `always_on` rules load unconditionally. The conventions are grounded in the products' documentation: [Codex `AGENTS.md`](https://learn.chatgpt.com/docs/agent-configuration/agents-md), [Antigravity rules](https://antigravity.google/docs/rules/), [Claude Code memory](https://code.claude.com/docs/en/memory), [GitHub Copilot repository custom instructions](https://docs.github.com/en/copilot/how-tos/configure-custom-instructions-in-your-ide/add-repository-instructions-in-your-ide), and [Cursor rules](https://cursor.com/docs/rules).
