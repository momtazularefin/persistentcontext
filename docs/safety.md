...
Translated directories are removed only when the projected post-apply inventory contains no remaining file, symlink, canonical write, relocation target, or preserved project material beneath them. Cleanup runs deepest first after file operations. Each removal is recorded in the write-ahead log; rollback recreates the directory tree before restoring earlier file preimages or moves, and the original inventory digest must match exactly.
