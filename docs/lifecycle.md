...
The first command is read-only. It compares the conversation checkpoint with globally newer active-event IDs. When nothing changed, plain output reports that immediately. When events are newer, it returns all of them with attribution, rationale, and affected paths, followed by the canonical context the agent must read. It never filters by workstream, inferred dependency, semantic scope, or path overlap.

Required context is deliberately narrower than the affected paths. An event's affected paths answer what changed; they are not an instruction to read every file any past increment touched, which on a mature project means hundreds of paths, including files inside nested repositories and paths that no longer exist. Required context is what PCP governs -- its own layer and the project documents `.pcp/state/documentation.yaml` catalogs -- filtered to paths that still exist. A first execution receives `.pcp/00-index.md` as its baseline entry.
