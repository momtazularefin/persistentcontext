...
describe('upgrade formatter', () => {
  it('renders preview and apply evidence for humans', () => {
    const preview = formatUpgrade({
      ...
      release_owned_paths: ['.pcp/pcp.yaml'],
      mechanical_migration_paths: [],
      agent_migration: {
        required: true,
        instruction_path: '.pcp/protocol/120-updates-and-reset.md',
        review_paths: ['.pcp/state/project.yaml'],
        instructions: ['Review project-derived state.'],
        completion_commands: ['node .pcp/tools/pcp.mjs validate . --json'],
      },
      history_purge: {
