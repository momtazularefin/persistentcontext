...
3. If no durable profile exists for your cached agent ID, register once. Adoption itself creates no profile and no event.
4. Read only the knowledge, operations, and project documents needed for the active scope.

## Returning task

1. Reuse the cached project-lifetime agent ID; do not recalculate it.
2. Compare the scoped checkpoint with newer journal events.
...
6. [Agents](agents/00-index.md) and [journal](journal/00-index.md) — identity and scoped reconciliation.
