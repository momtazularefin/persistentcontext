...
`inspect`, State A/B/C `adopt`, `register`, `status`, `record`, `validate`, `render`, `workstream`, and generated-adapter `repair` are available. Adoption and repair are preview-only unless the exact recomputed plan digest is supplied. Every adoption plan includes the same five generated platform adapters. Repair plans only missing or changed generated adapters in an otherwise valid managed project; preimages, collisions, the full inventory, live validation, and rollback remain transaction-bound. Use `render` for the declared generated status view. `register` recovers or creates durable identity; `status` advances one ignored checkpoint only after exact digest acknowledgement; `record` appends one attributed immutable event; and `workstream` atomically updates canonical state, its view, and continuity. Ordinary operations inspect archive filenames but not archive contents. An unimplemented adapter surface blocks State C adoption. `upgrade` returns `PCP_OPERATION_UNAVAILABLE`. Do not simulate an unavailable operation or mutate around the engine.
